...
function addStyle(typeTetromino) {
  ...
  const o = createTypeIdxArr('o').forEach((idxFromMesh, idx, arr) => {
    const columnIdx = idx % 4;
    mesh[idxFromMesh].classList.add(`o-${wordIdx[columnIdx]}`);
  });
  const i = createTypeIdxArr('i').forEach((idxFromMesh, idx, arr) => {
    const columnIdx = idx % 4;
    mesh[idxFromMesh].classList.add(`i-${wordIdx[columnIdx]}`);
  });
  const s = createTypeIdxArr('s').forEach((idxFromMesh, idx, arr) => {
    const columnIdx = idx % 4;
    mesh[idxFromMesh].classList.add(`s-${wordIdx[columnIdx]}`);
  });
  const z = createTypeIdxArr('z').forEach((idxFromMesh, idx, arr) => {
    const columnIdx = idx % 4;
    mesh[idxFromMesh].classList.add(`z-${wordIdx[columnIdx]}`);
  });
  const t = createTypeIdxArr('t').forEach((idxFromMesh, idx, arr) => {
    const columnIdx = idx % 4;
    mesh[idxFromMesh].classList.add(`t-${wordIdx[columnIdx]}`);
  });
  const j = createTypeIdxArr('j').forEach((idxFromMesh, idx, arr) => {
    const columnIdx = idx % 4;
    mesh[idxFromMesh].classList.add(`j-${wordIdx[columnIdx]}`);
  });
  const l = createTypeIdxArr('l').forEach((idxFromMesh, idx, arr) => {
    const columnIdx = idx % 4;
